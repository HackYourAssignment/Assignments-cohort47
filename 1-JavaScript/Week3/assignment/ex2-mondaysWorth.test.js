...
function computeEarnings(tasks, perHour) {
  const getEarning = tasks.map(function (task) {
    return (task.duration / 60) * perHour;
  });

  const sum = getEarning.reduce((total, currentValue) => total + currentValue);
